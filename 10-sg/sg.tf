module "mysql_sg" {
    source = "git::https://github.com/Kavyas099/terraform-aws-security-group.git?ref=main"
...
module "alb_ingress_sg" {
    ...
    environment = var.environment
    sg_name = "app_alb"
    sg_description = "Created for ingress instances in expense dev"
    vpc_id = data.aws_ssm_parameter.vpc_id.value
    common_tags = var.common_tags
}
...
module "eks_control_plane_sg" {
    ...
    sg_description = "Created for control plane in expense dev"
    vpc_id = data.aws_ssm_parameter.vpc_id.value
    common_tags = var.common_tags
}
...
module "eks_node_sg" {
    ...
    environment = var.environment
    sg_name = "eks-node-sg"
    sg_description = "Created for node eks in expense dev"
    vpc_id = data.aws_ssm_parameter.vpc_id.value
    common_tags = var.common_tags
}
